...
func TestInstallHookByIDInstallsBothSessionStartAndNotificationForTrackingHook(t *testing.T) {
	home := withFakeClaudeHome(t)
...
func TestIsHookInstalledByIDFalseWhenOnlyOneEventPresent(t *testing.T) {
	home := withFakeClaudeHome(t)
	path, _ := scriptPath("session-start.sh")
...
func TestInstallHookByIDSelfHealsPartialInstall(t *testing.T) {
	home := withFakeClaudeHome(t)
	path, _ := scriptPath("session-start.sh")
	writeRealSettings(t, home, fmt.Sprintf(`{
		"hooks": {
	...
	installed, err := IsHookInstalledByID(sessionTrackingID)
	if err != nil || !installed {
		t.Fatalf("IsHookInstalledByID after self-heal: installed=%v err=%v", installed, err)
	}
	settings := readSettingsRaw(t, home)
	hooks := settings["hooks"].(map[string]any)
...
func TestUninstallHookByIDRemovesBothEventsForTrackingHook(t *testing.T) {
	withFakeClaudeHome(t)
	if err := InstallHookByID(sessionTrackingID, "http://localhost:8787"); err != nil {
		t.Fatalf("InstallHookByID: %v", err)
	}
	...
	installed, err := IsHookInstalledByID(sessionTrackingID)
	if err != nil || installed {
		t.Fatalf("IsHookInstalledByID after uninstall: installed=%v err=%v, want false", installed, err)
	}
}
