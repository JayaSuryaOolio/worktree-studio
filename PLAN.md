...
7. **UI overhaul**: persistent left sidebar (worktree list, auto-loads per repo, shared `RepoContext` instead of `Workspace.tsx`'s own polling), a Cmd+K command palette (`cmdk`) for jumping between repos/worktrees and triggering "New worktree" without a full page, a "Command Deck" visual design pass (named color/type tokens, flight-strip-styled sidebar rows — see `docs/design.md` once written), and a dockview-based terminal arrangement replacing the plain tab strip: a "+ New Terminal ▾" dropdown with **New tab** (classic full-size tab switching within a group), **Split right**, **Split down** — real resizable split panels, persisted server-side (`worktree_layouts` table + `GET/PUT .../layout`). Explicitly NOT built, ever (not even as a TODO): OS-level popout into a separate browser window. Explicitly deferred as a TODO: theme switching (Command Deck is the only theme; token structure makes adding alternates cheap later, just not built now). Verify per sub-step — see the detailed plan this step was executed from (a UI-overhaul planning session); each sub-step gets its own commit(s), docs update, and PROGRESS.md entry per this file's own commit-checkpoint policy.
